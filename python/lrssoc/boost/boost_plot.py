...
class Plot:
    ...
    def measurements(self, data, t=None, fig=None):
        ...
        axes[0].step(t, data[:, 12], label='$i_{out} filt$', where='post', lw=self._lw)
        #axes[0].step(t, data[:, 15], label='$i_l filt$', where='post', lw=self._lw)
        ...
        axes[1].step(t, data[:, 3], label='$V_{out}$', where='post', lw=self._lw)
        axes[1].step(t, data[:, 9], label='$V_{out} reference$', where='post', lw=self._lw)
