
#ifndef BOOST_SBP_HW_IF_H_
#define BOOST_SBP_HW_IF_H_

//=============================================================================
/*-------------------------------- Includes ---------------------------------*/
//=============================================================================
#include "stdint.h"
//=============================================================================

//=============================================================================
/*------------------------------- Definitions -------------------------------*/
//=============================================================================
typedef enum{
    /**
     * Command
     *  -------------------------------------------
     *  | SET PWM RESET (uint32) | RESET (uint32) |
     *  -------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - The value of RESET should be either 0 or 1. A value of 1 places the
     *   PWM under reset (the PWM becomes inactive). A value of 0 releases the
     *   PWM for operation.
     */
    BOOST_SBP_HW_IF_SET_PWM_RESET,

    /**
     * Command
     *  --------------------------
     *  | GET PWM RESET (uint32) |
     *  --------------------------
     *
     * Response
     *  ------------------
     *  | RESET (uint32) |
     *  ------------------
     */
    BOOST_SBP_HW_IF_GET_PWM_RESET,

    /**
     * Command
     *  ----------------------------------------------------
     *  | SET PWM OUTPUT ENABLE (uint32) | ENABLE (uint32) |
     *  ----------------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - ENABLE should be either 0 or 1. A value of 1 enables the PWM signals
     *   at the GPIOs, and the state of the GPIOs depends on the PWM signals.
     *   A value of 0 disables the PWM signals and the GPIOs are set to 0.
     */
    BOOST_SBP_HW_IF_SET_PWM_OUTPUT_ENABLE,

    /**
     * Command
     *  ----------------------------------
     *  | GET PWM OUTPUT ENABLE (uint32) |
     *  ----------------------------------
     *
     * Response
     *  --------------------------
     *  | OUTPUT ENABLE (uint32) |
     *  --------------------------
     */
    BOOST_SBP_HW_IF_GET_PWM_OUTPUT_ENABLE,

    /**
     * Command
     *  ---------------------------------------------------------
     *  | SET PWM OVF TRIGGER ENABLE (uint32) | ENABLE (uint32) |
     *  ---------------------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - ENABLE should be either 0 or 1. A value of 1 enables generation of an
     *   overflow signal, generated when the PWM counter reaches the maximum
     *   value. A value of 0 disables generation of the overflow signal. This
     *   signal is used to trigger the ADC. Thus, if disabled, no ADC
     *   conversions will be triggered, even if the ADC is enabled.
     */
    BOOST_SBP_HW_IF_SET_PWM_OVF_TRIGGER_ENABLE,

    /**
     * Command
     *  ---------------------------------------
     *  | GET PWM OVF TRIGGER ENABLE (uint32) |
     *  ---------------------------------------
     *
     * Response
     *  -------------------------------
     *  | OVF TRIGGER ENABLE (uint32) |
     *  -------------------------------
     */
    BOOST_SBP_HW_IF_GET_PWM_OVF_TRIGGER_ENABLE,

    /**
     * Command
     *  ---------------------------------------
     *  | SET PWM INV (uint32) | INV (uint32) |
     *  ---------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - INV should be either 0 or 1. PWM A and B outputs are XOR'ed with
     *   the INV bit.
     */
    BOOST_SBP_HW_IF_SET_PWM_INV,

    /**
     * Command
     *  ------------------------
     *  | GET PWM INV (uint32) |
     *  ------------------------
     *
     * Response
     *  --------------------
     *  | INV (uint32) |
     *  --------------------
     */
    BOOST_SBP_HW_IF_GET_PWM_INV,

    /**
     * Command
     *  -----------------------------------------
     *  | SET PWM MODE (uint32) | MODE (uint32) |
     *  -----------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - MODE should be either 0 or 1. In MODE 0, S1 and S2 are PWM'ed, and 
     *   S3 and S4 can be controlled as regular switches. In MODE 1, S3 and S4
     *   are PWM'ed, and S1 and S2 are regular switches.
     */
    BOOST_SBP_HW_IF_SET_PWM_MODE,

    /**
     * Command
     *  -------------------------
     *  | GET PWM MODE (uint32) |
     *  -------------------------
     *
     * Response
     *  ----------------
     *  | MODE (uint32) |
     *  ----------------
     */
    BOOST_SBP_HW_IF_GET_PWM_MODE,

    /**
     * Command
     *  -------------------------------------------
     *  | SET PWM SW LS (uint32) | STATE (uint32) |
     *  -------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - This sets the state of switch 2 or 4, depending on the mode of the PWM.
     */
    BOOST_SBP_HW_IF_SET_PWM_LS_SW,

    /**
     * Command
     *  --------------------------
     *  | GET PWM LS SW (uint32) |
     *  --------------------------
     *
     * Response
     *  ------------------
     *  | STATE (uint32) |
     *  ------------------
     */
    BOOST_SBP_HW_IF_GET_PWM_LS_SW,

    /**
     * Command
     *  -------------------------------------------
     *  | SET PWM HS SW (uint32) | STATE (uint32) |
     *  -------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - This sets the state of switch 1 or 3, depending on the mode of the PWM.
     */
    BOOST_SBP_HW_IF_SET_PWM_HS_SW,

    /**
     * Command
     *  --------------------------
     *  | GET PWM HS SW (uint32) |
     *  --------------------------
     *
     * Response
     *  ------------------
     *  | STATE (uint32) |
     *  ------------------
     */
    BOOST_SBP_HW_IF_GET_PWM_HS_SW,
    
    /**
     * Command
     *  ---------------------------------------------------
     *  | SET PWM FREQUENCY (uint32) | FREQUENCY (uint32) |
     *  ---------------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - FREQUENCY is given in Hz. Note that it may not be possible to generate
     *   arbitrary frequencies. Internally, the PWM period is computed by
     *   using the following relation:
     *
     *      PERIOD = PWM_CLOCK / (2 * FREQUENCY)
     *
     *   PERIOD must be an integer value and is always truncated. Thus, it is
     *   not possible to generate arbitrary PWM frequencies.
     */
    BOOST_SBP_HW_IF_SET_PWM_FREQ,

    /**
     * Command
     *  ------------------------------
     *  | GET PWM FREQUENCY (uint32) |
     *  ------------------------------
     *
     * Response
     *  ----------------------
     *  | FREQUENCY (uint32) |
     *  ----------------------
     *
     * Notes
     * - FREQUENCY is given in Hz.
     */
    BOOST_SBP_HW_IF_GET_PWM_FREQ,

    /**
     * Command
     *  ----------------------------------------
     *  | SET PWM DUTY (uint32) | DUTY (float) |
     *  ----------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - DUTY is given as a floating point value between 0 and 1. Internally,
     *   the PWM counter is set according to the following relation:
     *
     *          COUNTER = DUTY * PWM_PERIOD
     *
     *   COUNTER must be a integer, and is truncated, which limits the
     *   resolution of the actual duty cycle .
     */
    BOOST_SBP_HW_IF_SET_PWM_DUTY,

    /**
     * Command
     *  -------------------------
     *  | GET PWM DUTY (uint32) |
     *  -------------------------
     *
     * Response
     *  ----------------
     *  | DUTY (float) |
     *  ----------------
     *
     * Notes
     * - Duty-cycle is given as a value between 0 and 1.
     */
    BOOST_SBP_HW_IF_GET_PWM_DUTY,

    /**
     * Command
     *  ---------------------------------------------------
     *  | SET PWM DEAD TIME (uint32) | DEAD TIME (uint32) |
     *  ---------------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - DEAD TIME is given in nanoseconds, as a floating point value.
     *   Internally, the PWM counter is set according to the following
     *   relation:
     *
     *          COUNTER = DEADTIME * PWM_CLOCK
     *
     *   COUNTER must be a integer, and is truncated, which limits the
     *   resolution of the dead time. Dead time resolution can be determined as
     *   multiples of the clock period. For instance, if PWM_CLOCK is 100 MHz,
     *   resulting in a period of 10 ns, the dead time can only be multiples of
     *   10 ns.
     */
    BOOST_SBP_HW_IF_SET_PWM_DEAD_TIME,

    /**
     * Command
     *  -----------------------------
     *  | GET PWM DEAD TIME(uint32) |
     *  -----------------------------
     *
     * Response
     *  ---------------------
     *  | DEAD TIME (float) |
     *  ---------------------
     *
     * Notes
     * - PWM dead time, in nanoseconds.
     */
    BOOST_SBP_HW_IF_GET_PWM_DEAD_TIME,

    /**
     * Command
     *  ---------------------------------------------
     *  | SET ADC ENABLE (uint32) | ENABLE (uint32) |
     *  ---------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - The value of ENABLE should be either 0 or 1. A value of 1 disables the
     *   ADC, and a value of 1 enables it. However, a conversion only takes
     *   place when the ADC is triggered, either by the PWM overflow signal, or
     *   by a manual trigger.
     */
    BOOST_SBP_HW_IF_SET_ADC_ENABLE,

    /**
     * Command
     *  ---------------------------
     *  | GET ADC ENABLE (uint32) |
     *  ---------------------------
     *
     * Response
     *  -------------------
     *  | ENABLE (uint32) |
     *  -------------------
     *
     * Notes
     * - DUTY is given as a value between 0 and 1.
     */
    BOOST_SBP_HW_IF_GET_ADC_ENABLE,

    /**
     * Command
     *  ------------------------------------------------------
     *  | SET ADC MANUAL TRIGGER (uint32) | TRIGGER (uint32) |
     *  ------------------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - TRIGGER should be either 0 or 1.
     * - Setting the ADC manual trigger will trigger one conversion (if the
     *   ADC is enabled). TRIGGER must be cleared, and set again for another
     *   conversion. If TRIGGER is not cleared, no further conversions take
     *   place, even if the PWM overflow signal is enabled.
     */
    BOOST_SBP_HW_IF_SET_ADC_MANUAL_TRIG,

    /**
     * Command
     *  -----------------------------------
     *  | GET ADC MANUAL TRIGGER (uint32) |
     *  -----------------------------------
     *
     * Response
     *  --------------------
     *  | TRIGGER (uint32) |
     *  --------------------
     */
    BOOST_SBP_HW_IF_GET_ADC_MANUAL_TRIG,

    /**
     * Command
     *  -------------------------------------------------------
     *  | SET ADC INTERRUPT ENABLE (uint32) | ENABLE (uint32) |
     *  -------------------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - ENABLE should be either 0 or 1. A value of 1 enables generation of an
     *   interrupt signal, and a value of 0 disables it.
     */
    BOOST_SBP_HW_IF_SET_ADC_INT_ENABLE,

    /**
     * Command
     *  -------------------------------------
     *  | GET ADC INTERRUPT ENABLE (uint32) |
     *  -------------------------------------
     *
     * Response
     *  -------------------
     *  | ENABLE (uint32) |
     *  -------------------
     */
    BOOST_SBP_HW_IF_GET_ADC_INT_ENABLE,

    /**
     * Command
     *  -------------------------------------------------------
     *  | SET ADC SPI FREQUENCY (uint32) | FREQUENCY (uint32) |
     *  -------------------------------------------------------
     *
     * Response
     * No response
     *
     * Notes
     * - FREQUENCY is given in Hz. Internally, a counter that generates the
     *   SPI clock signals is computed by:
     *
     *          COUNTER = ADC_CLOCK / (2 * FREQUENCY)
     *
     *   COUNTER is truncated; therefore, arbitrary SPI frequencies cannot be
     *   generated.
     */
    BOOST_SBP_HW_IF_SET_ADC_SPI_FREQ,

    /**
     * Command
     *  ----------------------------------
     *  | GET ADC SPI FREQUENCY (uint32) |
     *  ----------------------------------
     *
     * Response
     *  ----------------------
     *  | FREQUENCY (uint32) |
     *  ----------------------
     *
     * Notes
     * - FREQUENCY is given in Hz.
     */
    BOOST_SBP_HW_IF_GET_ADC_SPI_FREQ,

    /**
     * Command
     *  ---------------------------------------------
     *  | SET INPUT PRECH (uint32) | STATE (uint32) |
     *  ---------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_INPUT_PRECH,

    /**
     * Command
     *  ----------------------------
     *  | GET INPUT PRECH (uint32) |
     *  ----------------------------
     *
     * Response
     *  ------------------------
     *  | INPUT PRECH (uint32) |
     *  ------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_INPUT_PRECH,

    /**
     * Command
     *  ---------------------------------------------
     *  | SET INPUT PRECH (uint32) | STATE (uint32) |
     *  ---------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_INPUT_DISCH,

    /**
     * Command
     *  ----------------------------
     *  | GET INPUT DISCH (uint32) |
     *  ----------------------------
     *
     * Response
     *  ------------------------
     *  | INPUT DISCH (uint32) |
     *  ------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_INPUT_DISCH,

    /**
     * Command
     *  ---------------------------------------------
     *  | SET INPUT SSR P (uint32) | STATE (uint32) |
     *  ---------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_INPUT_SSR_P,

    /**
     * Command
     *  ----------------------------
     *  | GET INPUT SSR P (uint32) |
     *  ----------------------------
     *
     * Response
     *  ------------------------
     *  | INPUT SSR P (uint32) |
     *  ------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_INPUT_SSR_P,

    /**
     * Command
     *  ---------------------------------------------
     *  | SET INPUT SSR N (uint32) | STATE (uint32) |
     *  ---------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_INPUT_SSR_N,

    /**
     * Command
     *  ----------------------------
     *  | GET INPUT SSR N (uint32) |
     *  ----------------------------
     *
     * Response
     *  ------------------------
     *  | INPUT SSR N (uint32) |
     *  ------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_INPUT_SSR_N,

    /**
     * Command
     *  ---------------------------------------------
     *  | SET INPUT PRECH (uint32) | STATE (uint32) |
     *  ---------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_OUTPUT_PRECH,

    /**
     * Command
     *  -----------------------------
     *  | GET OUTPUT PRECH (uint32) |
     *  -----------------------------
     *
     * Response
     *  -------------------------
     *  | OUTPUT PRECH (uint32) |
     *  -------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_OUTPUT_PRECH,

    /**
     * Command
     *  ----------------------------------------------
     *  | SET OUTPUT PRECH (uint32) | STATE (uint32) |
     *  ----------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_OUTPUT_DISCH,

    /**
     * Command
     *  -----------------------------
     *  | GET OUTPUT DISCH (uint32) |
     *  -----------------------------
     *
     * Response
     *  -------------------------
     *  | OUTPUT DISCH (uint32) |
     *  -------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_OUTPUT_DISCH,

    /**
     * Command
     *  ----------------------------------------------
     *  | SET OUTPUT SSR P (uint32) | STATE (uint32) |
     *  ----------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_OUTPUT_SSR_P,

    /**
     * Command
     *  -----------------------------
     *  | GET OUTPUT SSR P (uint32) |
     *  -----------------------------
     *
     * Response
     *  -------------------------
     *  | OUTPUT SSR P (uint32) |
     *  -------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_OUTPUT_SSR_P,

    /**
     * Command
     *  ----------------------------------------------
     *  | SET OUTPUT SSR N (uint32) | STATE (uint32) |
     *  ----------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_OUTPUT_SSR_N,

    /**
     * Command
     *  -----------------------------
     *  | GET OUTPUT SSR N (uint32) |
     *  -----------------------------
     *
     * Response
     *  -------------------------
     *  | OUTPUT SSR N (uint32) |
     *  -------------------------
     *
     */
    BOOST_SBP_HW_IF_GET_OUTPUT_SSR_N,

    /**
     * Command
     *  -------------------------------------------
     *  | SET INPUT SSR (uint32) | STATE (uint32) |
     *  -------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_INPUT_SSR,

    /**
     * Command
     *  --------------------------------------------
     *  | SET OUTPUT SSR (uint32) | STATE (uint32) |
     *  --------------------------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_SET_OUTPUT_SSR,

//    /**
//     * Command
//     *  ---------------------------------------------
//     *  | SET MEAS GAINS (uint32) | GAINS (N bytes) |
//     *  ---------------------------------------------
//     *
//     * Response
//     * No response
//     *
//     */
//    BOOST_SBP_HW_IF_SET_MEAS_GAINS,
//
//    /**
//     * Command
//     *  ---------------------------
//     *  | GET MEAS GAINS (uint32) |
//     *  ---------------------------
//     *
//     * Response
//     *  -------------------
//     *  | GAINS (N bytes) |
//     *  -------------------
//     *
//     */
//    BOOST_SBP_HW_IF_GET_MEAS_GAINS,

    /**
     * Command
     *  -------------------------
     *  | CLEAR STATUS (uint32) |
     *  -------------------------
     *
     * Response
     * No response
     *
     */
    BOOST_SBP_HW_IF_CLEAR_STATUS,

    /**
     * Command
     *  -----------------------
     *  | GET STATUS (uint32) |
     *  -----------------------
     *
     * Response
     *  -------------------
     *  | STATUS (uint32) |
     *  -------------------
     *
     */
    BOOST_SBP_HW_IF_GET_STATUS,

    BOOST_SBP_HW_IF_END
}boostSbpHwIfCommands_t;

//=============================================================================

//=============================================================================
/*-------------------------------- Functions --------------------------------*/
//=============================================================================
//-----------------------------------------------------------------------------
int32_t boostSbpHwIfInitialize(void);
//-----------------------------------------------------------------------------
int32_t boostSbpHwIf(void *in, uint32_t insize, void **out, uint32_t maxoutsize);
//-----------------------------------------------------------------------------
//=============================================================================

#endif /* BOOST_SBP_HW_IF_H_ */
